...
beat printed 1.1017. A predicted γ is only a dent after faces say
M is copositive at that φ_target.

Printed leading uses the same ceil-to-next-10^{-4} as
tighten_leading.py: 1.10130 prints as 1.1013.
...
def row_from_phi(R, n, phi, err_hi, target, note=""):
    ...
    return {
        ...
        "beats_1.1017_if_certified": bool(float(printed) < Q9_LEAD),
...
def main() -> None:
    ...
    rec = row_from_phi(
        ...
        base34["err_P_hi"],
        0.9114,
        note="optional probe on the n=34 matrix; not an n=35 dent",
    )
    ...
    rec = row_from_phi(
        ...
        base35["err_P_hi"],
        0.9115,
        note="primary leftover: n=35 mid-radius bins at aspect 10",
    )
    rec["P_max_hi"] = base35.get("P_max_hi")
    extras.append(rec)
    print(
        f"R=10.0 n=35  raise tgt=0.9115  γ={rec['gamma_pred']:.6f}  "
        f"1/γ={rec['inv_pred']:.6f}  printed={rec['printed_lead']}  "
        f"cut_ge_γ={rec['cut_ge_gamma']}"
    )
    rows.extend(extras)
    ...
    feasible = [
        r
        for r in rows
        if r["beats_1.1017_if_certified"]
        and r["cut_ge_gamma"]
        and r["R"] == 10.0
    ]
    best = (
    ...
    out = {
        "note": (
            ...
            "uses the notebook ceil-to-next-10^{-4}; 1.10130 prints as 1.1013. "
            "An n=34 target-0.9114 probe is not an n=35 dent."
        ),
        "q9_leading": Q9_LEAD,
        "fmin": FMIN,
        "best_split": best,
    ...
    if best:
        ...
        print("no predicted R=10 row prints below 1.1017")
    print("wrote", path)
