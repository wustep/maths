#!/usr/bin/env bash
# Replay q10. Exit 0 is a lift of q9's 1.1017, or a recorded residue.
set -euo pipefail
cd "$(dirname "$0")"
mkdir -p certs work

echo "==> dead line: R<=9 cut cannot beat 1.1017"
...
if ls certs/raise_*.json >/dev/null 2>&1; then
  ...
  echo "q10 PASS (leading 1.1017 lifted to 1.1013)"
else
  echo "q10 PASS (residue so far; no certified raise_*.json)"
fi
