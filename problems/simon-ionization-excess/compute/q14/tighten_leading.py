#!/usr/bin/env python3
"""HPS Section 7 chain with the q14 finite-range compact gamma.

Same arithmetic as q1/tighten_hps.py and q11/tighten_leading.py.
The beta_3 lower bound comes from certs/span_bound.json.
...
def main() -> None:
    ...
    span = json.loads((CERTS / "span_bound.json").read_text())
    if not span.get("ok"):
        raise SystemExit("q14 span certificate is not valid")
    gamma = iv.mpf(str(span["beta3_lower"]))
    ...
    beats_q13 = bool(lead < mpf("1.1006") and lead_ok)
    ...
    blob = {
        "arxiv": "2504.18487v1",
        "beta3_lower": str(span["beta3_lower"]),
        "source": (
            f"q14 finite-range tax on q13 compact R={span['R']} "
            f"n={span['n']} target={span['target']}, lifted by mass-opt dichotomy"
        ),
        "row": "certs/span_bound.json",
        "R_split": int(span["R"]),
        ...
        "inequalities": [
            {
                "beats": (
                    "q13 leading 1.1006, q11 leading 1.1010, q10 1.1013, q9 1.1017, q8 1.1020, q7 1.1021, q6 1.1026, "
            ...
            {
                "beats": "q13 simplified 1.1006 Z + 3.933 Z^{1/3}",
                "range": "Z >= 4",
        ...
        "note": (
            ...
            f"aspect-{int(span['R'])} compact certificate, the finite "
            f"F-range reweighting tax, and the mass-stationary cut "
            f"Q>{int(span['R'])}/{int(span['R'])+1}. "
    ...
    print("wrote", out)
    if not (
        lead_ok and a1_ok and rem_ok and extras_ok and r_ok and beats_q13 and beats_hps
    ):
