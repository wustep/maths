...
quotient is at least \(0.9119\) for aspect \(R=10\) and \(n=37\)
geometric bins. Its continuous reweighting step used the coarse range
\(f_{\min}\leq\lambda_i\leq1\), costing

\[
P(1-f_{\min}),\qquad P=\frac{q-1}{q+1},\quad q=10^{1/37}.
\]

The same proof has a smaller finite range. If \(F_{ij}\) is the
minimum of \(f(t)=(1+t^3)/(1+t^2)\) on a bin pair, then

\[
f_{\min}\leq F_{ij}\leq f(q^2/10)<1.
\]

Indeed, below the minimizer \(t_*\), each bin-pair right endpoint is
at least \(q^2/10\); above \(t_*\), each left endpoint is at most
\(1/q\); and interval arithmetic checks
\(f(1/q)<f(q^2/10)\). Since every \(\lambda_i\) is a convex
combination of one row of \(F\), the same total-variation lemma costs
only
...
This is a dent of q13's printed \(1.1006\) leading coefficient. It
