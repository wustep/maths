// Second-language check of the aspect algebra and kernel identities.
...
fn main() {
    ...
    let q = (m1 * v1 + m2 * v2) / d;
    let e1 = (v1 - (d / 2.0 + mm1 / 2.0)).abs();
    let e2 = (v2 - (r2 * r2 / 2.0 + m3 / (2.0 * r2))).abs();
    assert!(e1 < 1e-12);
    assert!(e2 < 1e-12);
    assert!(q > 12.0 / 13.0);
    ...
    writeln!(
        f,
        "{{\"empty_bad\":{empty_bad},\"nonempty_bad\":{nonempty_bad},\"two_atom_Q\":{q:.16e},\"V1_err\":{e1:.16e},\"V2_err\":{e2:.16e},\"ok\":true}}"
    )
    .unwrap();
    println!("verify_aspect.rs PASS  two-atom Q={q:.6}  cut={:.6}", 12.0 / 13.0);
}
