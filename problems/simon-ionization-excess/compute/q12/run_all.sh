#!/usr/bin/env bash
# Replay q12. Exit 0 is a lift of q11's 1.1010, or a recorded residue.
set -euo pipefail
cd "$(dirname "$0")"
mkdir -p certs work
...
if ls certs/raise_*.json >/dev/null 2>&1; then
  ...
  echo "q12 PASS (leading 1.1010 lifted)"
else
  echo "q12 PASS (residue so far; no certified raise_*.json)"
fi
