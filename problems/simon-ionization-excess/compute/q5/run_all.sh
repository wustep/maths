#!/usr/bin/env bash
# Replay q5. Exit 0 is a dent of q4's 1.1057, or a recorded residue.
set -euo pipefail
cd "$(dirname "$0")"
mkdir -p certs work

echo "==> dead line: R<=9 cut cannot beat 1.1057"
...
if ls certs/raise_*.json >/dev/null 2>&1; then
  ...
  echo "q5 PASS (leading 1.1057 lifted to 1.1035)"
else
  echo "q5 PASS (residue so far; no certified raise_*.json)"
fi
