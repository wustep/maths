#!/usr/bin/env bash
# Replay q7. Exit 0 is a dent of q6's 1.1026, or a recorded residue.
set -euo pipefail
cd "$(dirname "$0")"
mkdir -p certs work

echo "==> dead line: R<=9 cut cannot beat 1.1026"
...
if ls certs/raise_*.json >/dev/null 2>&1; then
  ...
  echo "q7 PASS (leading 1.1026 lifted to 1.1021)"
else
  echo "q7 PASS (residue so far; no certified raise_*.json)"
fi
