...
beat printed 1.1013. A predicted γ is only a dent after faces say
M is copositive at that φ_target.

Printed leading uses the same ceil-to-next-10^{-4} as
tighten_leading.py: 1.10120 prints as 1.1012.
...
def row_from_phi(R, n, phi, err_hi, target, note=""):
    ...
    return {
        ...
        "beats_1.1013_if_certified": bool(float(printed) < Q10_LEAD),
...
def main() -> None:
    ...
    rec = row_from_phi(
        ...
        base34["err_P_hi"],
        0.9114,
        note="optional probe on the n=34 matrix; not an n=36 dent",
    )
    rec["P_max_hi"] = base34.get("P_max_hi")
    extras.append(rec)
    print(
        f"R=10.0 n=34  raise tgt=0.9114  γ={rec['gamma_pred']:.6f}  "
        f"1/γ={rec['inv_pred']:.6f}  printed={rec['printed_lead']}  "
    ...
    rec = row_from_phi(
        ...
        base35["err_P_hi"],
        0.9116,
        note="optional probe on the n=35 matrix; not an n=36 dent",
    )
    ...
    # default floor target already recorded; keep an explicit primary note
    rec = dict(base36)
    rec["note"] = "primary leftover: n=36 mid-radius bins at aspect 10"
    extras.append(rec)
    print(
        f"R=10.0 n=36  primary tgt={rec['target_suggest']:.4f}  "
    ...
    feasible = [
        r
        for r in rows
        if r["beats_1.1013_if_certified"]
        and r["cut_ge_gamma"]
        and r["R"] == 10.0
        and r["n"] == 36
    ]
    ...
    out = {
        "note": (
            ...
            "uses the notebook ceil-to-next-10^{-4}; 1.10120 prints as 1.1012. "
            "An n=34 or n=35 higher-target probe is not an n=36 dent."
        ),
        "q10_leading": Q10_LEAD,
        "fmin": FMIN,
        "best_split": best,
        "rows": rows,
    }
    ...
    if best:
        ...
        print("no predicted R=10 n=36 row prints below 1.1013")
    print("wrote", path)
