 ...
 *
 * Usage: verify_beta3 [matrix.txt] [faces.txt] [mask_lo mask_hi]
 *        [seed_minM seed_minPhi seed_interior seed_singular seed_mask_lo]
 *
 * Optional mask_lo mask_hi restrict the face loop to that inclusive
 * range (1-based Gray-free bitmask). Vertices (k=1) are always
 * scanned. Optional seeds resume a live shard: keep minM>0 and
 * continue from mask_lo. Checkpoints overwrite the faces path
 * every 2^22 masks. Build: gcc -O3 -o verify_beta3 verify_beta3.c -lm
 */
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
...
static void write_faces(const char *path, unsigned long long nfaces,
                        unsigned long long dump_lo, unsigned long long dump_hi,
                        unsigned long long interior, unsigned long long singular,
                        double min_val, double min_phi) {
    ...
    fprintf(out, "mask_lo %llu\n", dump_lo);
    fprintf(out, "mask_hi %llu\n", dump_hi);
...
int main(int argc, char **argv) {
    ...
    unsigned long long mask_lo = 1ULL;
    unsigned long long mask_hi = nfaces;
    unsigned long long dump_lo = 1ULL;
    if (argc > 4) {
        mask_lo = strtoull(argv[3], NULL, 10);
        mask_hi = strtoull(argv[4], NULL, 10);
        if (mask_lo < 1ULL)
            mask_lo = 1ULL;
        if (mask_hi > nfaces)
            mask_hi = nfaces;
    ...
    if (argc > 9) {
        min_val = atof(argv[5]);
        min_phi = atof(argv[6]);
        interior = strtoull(argv[7], NULL, 10);
        singular = strtoull(argv[8], NULL, 10);
        dump_lo = strtoull(argv[9], NULL, 10);
        if (dump_lo < 1ULL)
            dump_lo = 1ULL;
    }
    ...
    for (mask = mask_lo; mask <= mask_hi; mask++) {
        if ((mask & ((1ULL << 22) - 1)) == 0) {
            fprintf(stderr, "  ... mask %llu / %llu  minM=%.4e\n",
                    (unsigned long long)mask, (unsigned long long)nfaces,
                    min_val);
            fflush(stderr);
            write_faces(faces_path, nfaces, dump_lo, mask, interior, singular,
                        min_val, min_phi);
        }
        int idx[NMAX];
        int k = 0;
    ...
    double min_phi_safe = min_phi - MARGIN;
    int ok = (min_val_safe >= 0.0);
    write_faces(faces_path, nfaces, dump_lo, mask_hi, interior, singular,
                min_val, min_phi);
