#!/usr/bin/env bash
# Replay q11. Exit 0 is a lift of q10's 1.1013, or a recorded residue.
set -euo pipefail
cd "$(dirname "$0")"
mkdir -p certs work

echo "==> dead line: R<=9 cut cannot beat 1.1013"
...
if ls certs/raise_*.json >/dev/null 2>&1; then
  ...
  echo "q11 PASS (leading 1.1013 lifted)"
else
  echo "q11 PASS (residue so far; no certified raise_*.json)"
fi
