...
Not a certificate. Against printed 1.1013 the cut must exceed
1/1.1013 ≈ 0.908018.
...
def main() -> None:
    ...
    for R in (8.0, 9.0, 9.5, 9.8, 9.9, 10.0):
        ...
        rec = {
            ...
            "chebyshev_cut": ch,
            "need_vs_1.1013": NEED,
            "chebyshev_beats_1.1013": bool(ch is not None and ch > NEED),
            "positivity_beats_1.1013": bool(pos > NEED),
        }
        rows.append(rec)
        if R <= 9.0 and rec["chebyshev_beats_1.1013"]:
            reopen = True
        print(
            f"R={R:4.1f}  pos={pos:.6f}  cheb={ch}  "
            f"need>{NEED:.6f}  reopen={rec['chebyshev_beats_1.1013'] and R<=9}"
        )
    blob = {
        ...
        "reason": (
            ...
            "still below 0.908018, so R<=9 stays dead against printed "
            "1.1013. At R=9.5 the Chebyshev cut 0.91029 would sit above "
