#!/usr/bin/env bash
# Replay q8. Exit 0 is a dent of q7's 1.1021, or a recorded residue.
set -euo pipefail
cd "$(dirname "$0")"
mkdir -p certs work

echo "==> dead line: R<=9 cut cannot beat 1.1021"
...
if ls certs/raise_*.json >/dev/null 2>&1; then
  ...
  echo "q8 PASS (leading 1.1021 lifted to 1.1020)"
else
  echo "q8 PASS (residue so far; no certified raise_*.json)"
fi
