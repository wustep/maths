#!/usr/bin/env bash
# Replay q9. Exit 0 is a lift of q8's 1.1020, or a recorded residue.
set -euo pipefail
cd "$(dirname "$0")"
mkdir -p certs work

echo "==> dead line: R<=9 cut cannot beat 1.1020"
...
if ls certs/raise_*.json >/dev/null 2>&1; then
  ...
  echo "q9 PASS (leading 1.1020 lifted to 1.1017)"
else
  echo "q9 PASS (residue so far; no certified raise_*.json)"
fi
