...
def check_drat(drat_trim: Path, cnf: Path, proof: Path, log: Path) -> bool:
...
def main() -> int:
    ...
    if status == "UNSAT":
        ...
        if not check_drat(args.drat_trim, cnf, raw_proof, ROOT / "logs" / f"{case['name']}_drat.txt"):
            raise RuntimeError(f"{case['name']}: raw DRAT failed to verify")
        # Re-trim to a compact stored proof if the raw file is large.
        subprocess.run(
        ...
        if not check_drat(
            args.drat_trim, cnf, stored_src, ROOT / "logs" / f"{case['name']}_drat.txt"
        ):
