...
q2 stopped a pinned even BFS at 400,000 collections with 393,239
still in queue. This continues that neighbourhood.
...
Modes
  ...
  bfs     <out.jsonl> [limit]   exhaustive even add/drop/swap BFS
    ...
    while q and lg.evals < limit:
        ...
        for nw in even_neighbours(ids, adj, emask, pin_odd=pin_odd):
            ...
            if lg.evals >= limit:
                break
        if lg.evals % 200 == 0:
            print(f"  bfs evals={lg.evals} queue={len(q)} seen_coll={len(seen)} "
