...
seven odd splits, so size 5 is the next enumerable odd skeleton.

usage: python3 q4/odd_skel.py <maxsize> <out.jsonl> [max_evals]
"""
import json
import os
import sys
...
def main():
    maxsize = int(sys.argv[1]) if len(sys.argv) > 1 else 5
    out = resolve_out(sys.argv[2] if len(sys.argv) > 2
                      else "even_out/odd_skel5.jsonl")
    ...
    if maxsize >= 5 and not stop:
        for a in range(len(odds)):
            ...
            if stop:
                break
            if a % 5 == 0:
                print(f"  size5 {a}/{len(odds)} evals={evals} "
                      f"schemes={len(seen_sch)} hits={hits}", flush=True)
