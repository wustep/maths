...
two-split ladder around the twelve depth-3 M-collections. q2/q3 ran
the five (19,3) seeds under a time cap. This finishes that distance:
from each seed, drop at most one split and add three compatible splits.

A hit in collection space is a PL curve, not a T-curve, until
``haas.regularize`` plus exact ``tcurve.check_convexity``.

usage: python3 q4/ladder3.py <out.jsonl> [seconds] [seeds=193|depth3|all]
seconds=0 means no time cap. Complete only if every chosen seed finishes.
"""
import json
import os
import sys
...
def main():
    ...
    print(f"ladder3 seeds={len(keep)} which={which} "
          f"seconds={'none' if deadline is None else seconds}",
          flush=True)
    known = known_schemes()
    f = open(out, "w")
    seen = {}
    ...
    for name, ids, s0 in keep:
        ...
        if timed_out:
            break
        finished_seeds += 1
    complete = finished_seeds == len(keep)
    f.write(json.dumps({"kind": "summary", "evals": evals,
                        ...
                        "seeds": len(keep), "finished_seeds": finished_seeds,
                        "complete": complete,
