...
def count_size(k):
    """Count compatible odd k-tuples. k in {5,6,7}."""
    sp = dn.splits()
            ...
            for x in range(len(cand)):
                ...
                for y in range(len(c2)):
                    ...
                    for z in range(len(c3)):
                        ell = c3[z]
                        ml = mk & adj[ell]
                        c4 = [c3[w] for w in range(z + 1, len(c3))
                              if (ml >> c3[w]) & 1]
                        for w in range(len(c4)):
                            p = c4[w]
                            mp = ml & adj[p]
                            n_a += sum(1 for t in range(w + 1, len(c4))
                                       if (mp >> c4[t]) & 1)
        elif k == 7:
            for x in range(len(cand)):
                j = cand[x]
                ...
                for y in range(len(c2)):
                    ...
                    for z in range(len(c3)):
                        ...
                        for w in range(len(c4)):
                            p = c4[w]
                            mp = ml & adj[p]
                            c5 = [c4[t] for t in range(w + 1, len(c4))
                                  if (mp >> c4[t]) & 1]
                            for t in range(len(c5)):
                                q = c5[t]
                                mq = mp & adj[q]
                                n_a += sum(1 for u in range(t + 1, len(c5))
                                           if (mq >> c5[u]) & 1)
        else:
            raise SystemExit(f"unsupported k={k}")
        per.append(n_a)
        total += n_a
        if a % 10 == 0:
            print(f"  count{k} a={a}/{len(odds)} n={n_a} total={total}",
                  flush=True)
    print(json.dumps({
        "odd_splits": len(odds),
        f"size{k}_tuples": total,
        "per_first": per,
...
def main(argv):
    ...
    if args.maxsize_or_count == "count":
        count_size5()
        return
    if args.maxsize_or_count in ("count6", "count7"):
        count_size(int(args.maxsize_or_count[-1]))
        return
