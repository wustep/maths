...
from common import (HERE, LEFTOVER_PLAN_SHA, LEFTOVER_SEEDS_SHA, baseline,
                    digest, leftover_plan, leftover_seeds)
...
def main():
    ...
    assert saved['planned'] == len(rows) == 106
    assert len(saved['tasks']) == 106
    assert saved['evaluations'] == 106 * expected
    ids = [t['id'] for t in saved['tasks']]
    assert ids == list(range(106))
    novel = set()
    ...
    print(f'Coverage audit: leftover {saved["n_tasks"]}/106, '
