...
from common import (HERE, ROOT, PTS, Q8, Q9, FOLLOWUP_SCHEMES, baseline, canon,
                    code_scheme, flips, leftover_seeds, parse_pcom, q9_schemes,
                    scheme_code, seeds, unpack, write_task)
...
def main():
    ...
    for cert in q9_certificates():
        both(cert, exe)
    print('PASS: Python and Rust agree on all 55 seeds, q8, and nine q9 schemes', flush=True)
    ...
    assert len(baseline()) == 2394
    assert len(q9_schemes()) == 9
    print('PASS: archive TYPE, filenames and replayed B agree; baseline has 2,394', flush=True)
    ...
    reject(lambda: verify_new(leftover[0], exe), 'leftover q9 followup seed')
    ...
    print('PASS: known schemes, q8/q9 schemes, false target, and malformed geometry/signs rejected', flush=True)
