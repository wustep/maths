#!/usr/bin/env python3
"""Two independent geometry/topology checks and the exact q10 novelty gate."""
import argparse
...
def verify_new(cert, exe):
    ...
    assert got not in baseline(), 'scheme already in the 2,394 baseline'
    return got
...
def main():
    ...
    if not path.exists():
        print('RESIDUE: no candidate certificate; q10 CLAIM is not established.')
        return 2
    ...
    print(f'PASS: {len(found)} new nonempty schemes; count >= {2394 + len(found)}')
    return 0
