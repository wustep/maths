#!/usr/bin/env python3
"""Bounded sign balls on the leftover q9-followup one-flip plan.
...
from common import (HERE, LEFTOVER_PLAN_SHA, LEFTOVER_SEEDS_SHA, atomic_json,
                    ...
                    digest, leftover_plan, leftover_seeds, source_hashes,
                    task_tris, unpack, write_task)
...
def main():
    ...
    ap.add_argument('--stop', type=int, default=106)
    ...
    if args.certs:
        ...
        if not cpath.is_absolute():
            cpath = HERE / cpath
        rows, seedlist = (cert_plan(cpath, source_prefix='q10 addition: '),
                          cert_seeds(cpath, source_prefix='q10 addition: '))
        domain = 'q10-certificate-flips'
        extra_known = {canon(c['scheme']) for c in seedlist}
    else:
        rows, seedlist = leftover_plan(), leftover_seeds()
        domain = 'q9-followup-leftover'
