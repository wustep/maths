 ...
 * real scheme (same nesting logic as fastcx._scheme, with the bracket
...
int main(int argc, char **argv)
{
    ...
    if (!f) { perror("task"); return 1; }
    if (fscanf(f, "%d %d %d %d %d", &npts, &nv, &F, &nedges, &rnk) != 5) return 1;
    ...
    int *px = malloc(npts * sizeof(int)), *py = malloc(npts * sizeof(int));
    for (int i = 0; i < npts; i++) { need(f, 1, &px[i]); need(f, 1, &py[i]); }
    fclose(f);
