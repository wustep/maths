#!/usr/bin/env python3
"""Bounded sign balls on the leftover q11-followup one-flip plan.
...
def main():
    ...
    if args.certs:
        ...
        if not cpath.is_absolute():
            cpath = HERE / cpath
        rows, seedlist = (cert_plan(cpath, source_prefix='q12 addition: '),
                          cert_seeds(cpath, source_prefix='q12 addition: '))
        domain = 'q12-certificate-flips'
        extra_known = {canon(c['scheme']) for c in seedlist}
    else:
        rows, seedlist = leftover_plan(), leftover_seeds()
        domain = 'q11-followup-leftover'
