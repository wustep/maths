...
from common import (HERE, ROOT, PTS, Q8, Q9, Q10, Q11, FOLLOWUP_SCHEMES, baseline,
                    canon, code_scheme, flips, leftover_seeds, parse_pcom,
                    q11_schemes, scheme_code, seeds, unpack, write_task)
...
def q9_certificates():
    return json.loads((Q9 / 'certs/new_schemes.json').read_text())


def q10_certificates():
    return json.loads((Q10 / 'certs/new_schemes.json').read_text())


def q11_certificates():
    return json.loads((Q11 / 'certs/new_schemes.json').read_text())
...
def main():
    ...
    for cert in seedlist:
        both(cert, exe)
    for cert in q8_certificates():
        both(cert, exe)
    for cert in q9_certificates():
        both(cert, exe)
    for cert in q10_certificates():
        both(cert, exe)
    for cert in q11_certificates():
        both(cert, exe)
    print('PASS: Python and Rust agree on all 55 seeds, q8, nine q9, thirteen q10, and eight q11 schemes',
          flush=True)
    ...
    assert len(baseline()) == 2415
    assert len(q11_schemes()) == 8
    print('PASS: archive TYPE, filenames and replayed B agree; baseline has 2,415', flush=True)

    reject(lambda: verify_new(seedlist[0], exe), 'published scheme')
    reject(lambda: verify_new(seedlist[-1], exe), 'parent addition')
    reject(lambda: verify_new(q8_certificates()[0], exe), 'q8 scheme')
    reject(lambda: verify_new(q9_certificates()[0], exe), 'q9 scheme')
    reject(lambda: verify_new(q10_certificates()[0], exe), 'q10 scheme')
    reject(lambda: verify_new(q11_certificates()[0], exe), 'q11 scheme')
    leftover = leftover_seeds()
    reject(lambda: verify_new(leftover[0], exe), 'leftover q11 followup seed')
    ...
    print('PASS: known schemes, q8/q9/q10/q11 schemes, false target, and malformed geometry/signs rejected',
          flush=True)
    ...
    assert leftover_seeds()[0]['scheme'] == FOLLOWUP_SCHEMES[0]
