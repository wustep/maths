...
from common import (HERE, ROOT, PTS, Q8, baseline, canon, code_scheme, flips,
                    parse_pcom, q8_certificates, scheme_code, seeds, unpack,
                    write_task)
...
def main():
    ...
    for cert in q8_certificates():
        both(cert, exe)
    print('PASS: Python and Rust agree on all 55 seeds and the q8 scheme', flush=True)
    ...
    assert len(baseline()) == 2385
    print('PASS: archive TYPE, filenames and replayed B agree; baseline has 2,385', flush=True)
    ...
    print('PASS: known schemes, q8 scheme, false target, and malformed geometry/signs rejected', flush=True)
