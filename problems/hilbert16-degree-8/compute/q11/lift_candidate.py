...
from common import (HERE, PTS, atomic_json, baseline, canon, cert_plan,
                    cert_seeds, code_scheme, leftover_plan, leftover_seeds,
                    task_tris)
...
def load_row(task, certs=None):
    if certs:
        rows, seedlist = cert_plan(certs, source_prefix='q11 addition: '), cert_seeds(certs, source_prefix='q11 addition: ')
    else:
...
def main():
    ...
    if args.certs:
        ...
        if not cpath.is_absolute():
            cpath = HERE / cpath
    row, seed, tris = load_row(args.task, certs=cpath)
    candidates = None
    if args.shard:
        ...
        want = 'q11-certificate-flips' if args.certs else 'q10-followup-leftover'
