...
2,407-scheme baseline (2,367 published, seventeen parent additions,
the q8 scheme ⟨3 ⊔ 1⟨3⟩ ⊔ 1⟨12⟩⟩, the nine q9 schemes, and the
thirteen q10 schemes).

`controls.py --full-baseline` checks all published certificates,
the seventeen additions, the q8 scheme, the nine q9 schemes and the
thirteen q10 schemes with both implementations. Default controls
check the 55 seeds, q8, q9 and q10, reject those known schemes and
a falsely claimed open nest, and damage signs, heights, points and
...
zero only after the existence predicate in `CLAIM.md` passes.
