...
from common import (HERE, ROOT, PTS, Q8, Q9, Q10, FOLLOWUP_SCHEMES, baseline,
                    canon, code_scheme, flips, leftover_seeds, parse_pcom,
                    q10_schemes, scheme_code, seeds, unpack, write_task)
...
def main():
    ...
    for cert in q10_certificates():
        both(cert, exe)
    print('PASS: Python and Rust agree on all 55 seeds, q8, nine q9, and thirteen q10 schemes',
          flush=True)
    ...
    assert len(baseline()) == 2407
    assert len(q10_schemes()) == 13
    print('PASS: archive TYPE, filenames and replayed B agree; baseline has 2,407', flush=True)
    ...
    reject(lambda: verify_new(leftover[0], exe), 'leftover q10 followup seed')
    ...
    print('PASS: known schemes, q8/q9/q10 schemes, false target, and malformed geometry/signs rejected',
          flush=True)
