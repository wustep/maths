#!/usr/bin/env python3
"""Two independent geometry/topology checks and the exact q11 novelty gate."""
import argparse
...
def verify_new(cert, exe):
    ...
    assert got not in baseline(), 'scheme already in the 2,407 baseline'
    return got
...
def main():
    ...
    if not path.exists():
        print('RESIDUE: no candidate certificate; q11 CLAIM is not established.')
        return 2
    ...
    print(f'PASS: {len(found)} new nonempty schemes; count >= {2407 + len(found)}')
    return 0
