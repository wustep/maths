#!/usr/bin/env python3
"""Bounded sign balls on the leftover q10-followup one-flip plan.
...
from common import (HERE, LEFTOVER_PLAN_SHA, LEFTOVER_SEEDS_SHA, N_LEFTOVER,
                    atomic_json, baseline, canon, cert_plan, cert_seeds,
                    code_scheme, digest, leftover_plan, leftover_seeds,
...
def main():
    ...
    ap.add_argument('--stop', type=int, default=N_LEFTOVER)
    ...
    if args.certs:
        ...
        if not cpath.is_absolute():
            cpath = HERE / cpath
        rows, seedlist = (cert_plan(cpath, source_prefix='q11 addition: '),
                          cert_seeds(cpath, source_prefix='q11 addition: '))
        domain = 'q11-certificate-flips'
        extra_known = {canon(c['scheme']) for c in seedlist}
    else:
        rows, seedlist = leftover_plan(), leftover_seeds()
        domain = 'q10-followup-leftover'
