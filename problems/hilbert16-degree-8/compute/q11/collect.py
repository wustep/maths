...
def merge_leftover():
    ...
    for path, saved in shards:
        ...
        assert saved['provenance']['domain'] == 'q10-followup-leftover'
...
def merge_followup(exclude=()):
    ...
    for path, saved in shards:
        assert saved['provenance']['domain'] == 'q11-certificate-flips'
        assert saved['complete']
        if plan_sha is None:
...
def main():
    ...
    out = dict(leftover=leftover, followup=followup,
               q10_claim='../q10/CLAIM.md',
               baseline=2407,
               n_new=len(schemes),
               bound=2407 + len(schemes),
               schemes=schemes,
    ...
    if leftover['complete'] and not out['candidate_schemes']:
        print('RESIDUE: leftover domain finished with no scheme outside the 2,407.')
        return 2
