...
n=109 k=37 is the old leftover hole from the n=108 wrap. It is UNSAT, and so is every later leftover order through n=113. Independent replay of those 176 cubes is 0 failures. Raw kissat proofs again grow past 100 MB; `drat-trim` core lemmas still replay.
...
The stored proofs for this range are the 176 DRATs in `compute/q5/certs/keep/`. Replay regenerates each CNF and runs `drat-trim`. The first hole not run is n=114. The conjecture 1/3 is open.
