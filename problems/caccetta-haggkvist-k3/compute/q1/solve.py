...
def run_one(
    ...
    clauses, nvars = encode(
        n, d, exact=True, sb=sb, indeg0=indeg0, exact_in=exact_in
    )
    ...
    for line in text.splitlines():
        if "conflicts" in line.lower() and any(ch.isdigit() for ch in line):
            rec.setdefault("kissat_lines", [])
            if len(rec["kissat_lines"]) < 8:
                rec["kissat_lines"].append(line.strip())
