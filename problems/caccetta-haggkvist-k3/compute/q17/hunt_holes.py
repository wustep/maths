...
from holes import cube_range, remaining_after_q16
...
def write_summary(closed, residue, this_run):
    ...
    first_open = None
    leftover = remaining_after_q16(n_max=400)
...
def main():
    ...
    ap.add_argument("--n-min", type=int, default=126)
    ...
    holes = [
        row
        for row in remaining_after_q16(n_max=args.n_max)
        if row["n"] >= args.n_min
    ]
    print(
