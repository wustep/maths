#!/usr/bin/env python3
"""Walk remaining exact holes after q36, high-k cubes first, store DRATs.
...
def write_summary(closed, residue, this_run):
    ...
    first_open = None
    leftover = remaining_after_q36(n_max=400)
...
def main():
    ...
    ap.add_argument("--n-min", type=int, default=147)
    ...
    holes = [
        row
        for row in remaining_after_q36(n_max=args.n_max)
        if row["n"] >= args.n_min
    ]
    print(
