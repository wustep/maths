...
def solve_cube(n, d, k, secs, proof, exact_in, u_from_1, stable, keep):
...
def main():
    ...
    ap.add_argument("--d", type=int, default=13)
    ...
    k_max = args.k_max if args.k_max is not None else min(
        info["k_max_absolute"], info["k_max_count"]
    )
    ...
    needed = set(info["needed_cubes"])
    needed_ok = all(
        r["status"] == "UNSAT" and r.get("drat") == "VERIFIED"
        for r in rows
        if r["indeg0"] in needed
    )
    summary = {
        "n": args.n,
        "d": args.d,
        ...
        "pigeonhole": info,
        "all_unsat": all_unsat,
        "needed_drat_ok": needed_ok,
        "leftover": leftover,
        "rows": rows,
        "note": (
            "all needed cubes UNSAT with DRAT is a certificate of the exact statement"
            if needed_ok
    ...
    print("wrote", path, "all_unsat=", all_unsat, "needed_drat_ok=", needed_ok)
