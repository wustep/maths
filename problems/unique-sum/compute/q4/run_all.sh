...
"$PYTHON" "$HERE/run_cover.py" "$BUILD/cover" 59 14 --initial-ap 5 \
    --nodes 100000000 --seconds 600 --output "$BUILD/c-ap5.json"
"$PYTHON" - "$BUILD" <<'PY'
import json
from pathlib import Path
import sys
build = Path(sys.argv[1])
for name in ('rust-ap5.json', 'c-ap5.json'):
    result = json.loads((build/name).read_text())
    assert result['status'] == 'UNSAT', (name, result)
    assert result['p'] == 59 and result['bound'] == 14 and result['initial_ap'] == 5
    assert not result.get('forbid_ap', 0)
print('VALID_CLAIM: m(59)<=15; AP5 family excluded through size 14.')
