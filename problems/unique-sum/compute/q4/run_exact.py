#!/usr/bin/env python3
"""Run one exact-search binary with explicit wall, node and address-space caps.

A killed/limited run always records UNKNOWN. Only a clean UNSAT completion
is an exclusion. This harness does not itself certify the search algorithm.
"""
import argparse
import hashlib
import json
...
def main():
    ...
    command = [str(args.binary.resolve()), str(args.p), str(args.bound), str(args.nodes), str(args.initial_ap)]
    def cap_memory():
    ...
    report = dict(p=args.p, bound=args.bound, initial_ap=args.initial_ap, status=status,
