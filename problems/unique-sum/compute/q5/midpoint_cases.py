...
def main():
    ...
    ap.add_argument('--seconds-per-case', type=float, default=120)
    ...
    save()
    order = [i for i, row in enumerate(payload['cases']) if row.get('representative') == i]
    order.sort(key=lambda i: -len(payload['cases'][i]['root']))
    for i in order:
        row = payload['cases'][i]
        ...
        save()
        print(f"case={i} center={args.center} pair={row['pair']} "
              f"root_size={len(row['root'])} status={row['result']['status']}", flush=True)
