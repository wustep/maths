 ...
 *
 * Same completeness argument as q4: ordered counts are popcounts of
 * S intersect (s-S); a count of 1 or 2 forces another unordered pair.
 * Relative to q4, repair options are tried one-point first, and the
 * exact cover test runs at remaining <= 7. A cover-node cap may skip
 * a prune (never an exclusion). AP avoidance is affine invariant.
 */
...
typedef struct { int n, sum; Mask option[32]; } Constraint;
...
static void sort_options(Constraint *c) {
    for (int i = 1; i < c->n; ++i) {
        Mask x = c->option[i];
        int xb = bits(x), j = i;
        while (j) {
...
static int search(Mask mask) {
    ...
    for (int s = 0; s < p; ++s) {
        ...
        if (!c[n].n) return 0;
        sort_options(&c[n]);
        int diagonal = (mask >> ((s * ((p+1)/2)) % p)) & 1;
        int better = c[n].n < c[best].n;
        if (!better && c[n].n == c[best].n) {
            int best_diag = (mask >> ((c[best].sum * ((p+1)/2)) % p)) & 1;
            better = diagonal && !best_diag;
        }
        if (better) best = n;
        ++n;
    }
    ...
    cover_nodes = 0;
    if (remaining <= 7 && !cover(c, n, 0, remaining)) { ++cover_prunes; return 0; }
