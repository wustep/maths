/* Leftover 41-set hosted by a 4-star union of D5 coordinate-stars.

 * Same-missed extras are independent.  A clique E of extras with
 * missed-union U, together with D5 \ U, is a leftover 41-set iff
 * |U| >= 19 and |E| >= |U| + 1.  q5 emptied every 3-star host;
 * this file searches each of the 210 four-star pools.
 *
 * Usage: ./four_star_extras [target] [node_limit] [shard] [nshards]
...
static void write_code41(int rsz, const int *stack, u64 U)
{
    ...
    fprintf(f, "  \"source\": \"q6 four_star_extras.c\",\n");
            ...
            for (int c = b + 1; c < 10; c++) {
                for (int e = c + 1; e < 10; e++) {
                    if ((pidx++ % nshards) != shard)
                        continue;
                    u64 U4 = stars[a] | stars[b] | stars[c] | stars[e];
                    ...
                    if (!first_pool)
                        printf(",\n");
                    first_pool = 0;
                    printf("    {\"stars\": [%d, %d, %d, %d], \"k\": %d, "
                           "\"n_extras\": %d, \"best\": %d, \"nodes\": %ld, "
                           "\"complete\": %s, \"found_41\": %s}",
                           a, b, c, e, k, nL, best, nodes,
...
done_pools:
    ...
    printf("  \"tot_nodes\": %ld,\n", tot_nodes);
    printf("  \"found_41\": %s,\n", any41 ? "true" : "false");
    printf("  \"comment\": \"4-star leftover extras B&B; |E|>=20 and |E|>=|U|+1\"\n");
    printf("}\n");
    return 0;
}
