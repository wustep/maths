...
def forbid_unions(cnf, G, inst, combs):
    """At least one y_r outside each listed star-union."""
    stars = G["stars"]
...
def global_leftover_cnf(G, k=19, min_star_cover=5, forbid_five_types=None):
    """|U| = k, |E| >= k+1, U not in a (min_star_cover-1)-star.

    q7 emptied type (2,1) and (1,3) five-star leftover hosts.  Passing
    forbid_five_types=((2,1),(1,3)) adds those 220 unions as extra
    forbids.  Restricted; valid only after those orbit SAT certificates.
    """
    ...
    cnf = inst["cnf"]
    if min_star_cover >= 5:
        forbid_unions(cnf, G, inst, combinations(range(10), min_star_cover - 1))
    n_five = 0
    if forbid_five_types:
        wanted = {tuple(t) for t in forbid_five_types}
        combs = [c for c in combinations(range(10), 5)
                 if star_type(c) in wanted]
        n_five = forbid_unions(cnf, G, inst, combs)
    inst["min_star_cover"] = min_star_cover
    inst["forbid_five_types"] = [list(t) for t in (forbid_five_types or [])]
    inst["n_five_forbids"] = n_five
    inst["cnf"] = cnf
    return inst
