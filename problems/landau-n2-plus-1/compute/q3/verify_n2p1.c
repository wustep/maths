 ...
 *
 * Even-n membership is stored as bitsets (~6.25 MB each at n_max=1e8)
 * rather than a byte per even n.
 */
...
static void write_verify_ok(
    ...
    char path[4096];
    snprintf(path, sizeof path, "%s/verify_ok.json", dir);
    FILE *f = fopen(path, "w");
    if (!f) {
        perror(path);
        exit(1);
    ...
    fprintf(f, "  \"producer\": \"verify_n2p1.c\",\n");
    fprintf(f, "  \"algorithm\": \"miller-rabin every even n; trial<=2003 plus Pollard Omega\",\n");
    fprintf(f, "  \"seconds\": %.6f,\n", seconds);
    fprintf(f, "  \"primes_claimed\": %" PRIu64 ",\n", nclaimed);
    fprintf(f, "  \"primes_found\": %" PRIu64 ",\n", nfound);
    fprintf(f, "  \"extra\": %" PRIu64 ",\n", extra);
    fprintf(f, "  \"missing\": %" PRIu64 ",\n", missing);
    fprintf(f, "  \"p2_rows\": %" PRIu64 ",\n", np2);
    fprintf(f, "  \"complete_miss\": %" PRIu64 ",\n", miss_p2);
    fprintf(f, "  \"complete_extra\": %" PRIu64 "\n", extra_p2);
    fprintf(f, "}\n");
    fclose(f);
}
...
int main(int argc, char **argv) {
    ...
    if (!n_max) {
        snprintf(path, sizeof path, "%s/sieve_meta.json", dir);
        n_max = parse_nmax(path);
    }
    if (!n_max) {
        fprintf(stderr, "n_max not found in n2p1.json or sieve_meta.json\n");
        return 1;
    }
    uint64_t size = n_max / 2;
    size_t nbytes = (size_t)((size + 7) / 8);
    uint8_t *is_prime = calloc(nbytes, 1);
    uint8_t *is_p2 = calloc(nbytes, 1);
    if (!is_prime || !is_p2) {
        fprintf(stderr, "bitset calloc failed (%zu bytes each)\n", nbytes);
        return 1;
    }
    ...
    write_verify_ok(dir, n_max, nclaimed, nfound, extra, missing, np2, miss_p2, extra_p2, now_sec() - t0);
    printf("OK\n");
    free(line);
    free(is_prime);
    free(is_p2);
