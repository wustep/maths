/* Hamming distance 13 around the 367-set (6-out / 7-in), high-blocker cases.

   q2 finished odd Hamming distance 11. The 6-blocker vertices determine
   the removal 6-set (case A). A 5-blocker vertex plus one extra removal
   is case B. Cases that add only ≤4-blocker vertices are left to SAT
   or later work.

   gcc -O3 -o q3/search_hamming13 q3/search_hamming13.c
   ./q3/search_hamming13 R367.txt
*/
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char **argv) {
    ...
    for (int v = 0; v < NV; v++) {
        if (in_seed[v]) continue;
        int nb = nblock[v];
        if (nb >= 0 && nb < 16) by_nb[nb][n_by_nb[nb]++] = v;
    }

    int best_mis = 0, best_freed = 0, nA = 0, hit = 0;
    int freed[8192];
    ...
    int best_mis_B = 0, max_freed_B = 0, nB = 0;
    if (!hit) {
        int base_freed[8192];
        for (int t = 0; t < n_by_nb[5]; t++) {
            ...
            if ((t + 1) % 400 == 0) {
                printf("  caseB %d/%d best_mis=%d max_freed=%d t=%.1fs\n", t + 1,
                       n_by_nb[5], best_mis_B, max_freed_B,
    ...
    if (!hit)
        printf("no 368 in Hamming-13 cases A+B (added vertex with 5 or 6 "
               "blockers)\n");
    return 0;
}
