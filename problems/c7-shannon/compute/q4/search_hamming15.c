/* Hamming-15 high-blocker split: 7-out / 8-in.

   Case A: an added vertex has 7 blockers (determined 7-set).
   Case B: a 6-blocker plus one extra removal.
   Leftover: only ≤5-blocker adds.

   gcc -O3 -o q4/search_hamming15 q4/search_hamming15.c
   ./q4/search_hamming15 ../R367.txt
*/
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
static int exact_mis(const int *verts, int n) {
    ...
    if (n > 40) {
        int take = 0;
        uint8_t *used = calloc((size_t)n, 1);
        for (int i = 0; i < n; i++) {
            if (used[i]) continue;
            take++;
            for (int j = i + 1; j < n; j++)
...
int main(int argc, char **argv) {
    ...
    clock_t t0 = clock();
    fill_tables();
    if (load_seed(path) != SEEDN) return 1;
    ...
    for (int i = 0; i < SEEDN; i++)
        for (int k = 0; k < 243; k++) {
            ...
            if (!in_seed[u] && nblock[u] < 16) block_list[u][nblock[u]++] = i;
        }
    memset(n_by_nb, 0, sizeof n_by_nb);
    for (int v = 0; v < NV; v++)
        if (!in_seed[v] && nblock[v] < 16) n_by_nb[nblock[v]]++;
    printf("blocker_hist");
    ...
    for (int v = 0; v < NV; v++)
        if (!in_seed[v] && nblock[v] < 16) by_nb[nblock[v]][n_by_nb[nblock[v]]++] = v;

    int bestA = 0, maxA = 0, nA = 0, hit = 0;
    int freed[8192];
    for (int t = 0; t < n_by_nb[7]; t++) {
        int v = by_nb[7][t];
        uint8_t inR[SEEDN];
    ...
    int bestB = 0, maxB = 0, nB = 0;
    if (!hit) {
        int base_freed[8192];
        for (int t = 0; t < n_by_nb[6]; t++) {
            int v = by_nb[6][t];
            uint8_t baseR[SEEDN];
            ...
            for (int e = 0; e < SEEDN; e++) {
                ...
                for (int k = 0; k < 243; k++) {
                    ...
                    if (in_seed[u] || nblock[u] > 7) continue;
                    int ok = 1, uses_e = 0;
                    ...
                    if (ok && uses_e && nf < 8192) freed[nf++] = u;
                }
                int a = exact_mis(freed, nf);
                if (a > bestB) bestB = a;
                if (nf > maxB) maxB = nf;
    ...
    printf("DONE hit=%d caseA_best=%d caseB_best=%d t=%.2fs\n", hit, bestA, bestB,
           (double)(clock() - t0) / CLOCKS_PER_SEC);
    if (!hit)
        printf("no 368 in Hamming-15 cases A+B (added vertex with 6 or 7 "
               "blockers)\n");
    return 0;
}
