#!/usr/bin/env python3
"""Hamming-13 leftover after case C: 6-out / 7-in using only ≤3-blockers.

q3 SAT used ≤4-blocker candidates and did not finish. Case C (C search)
handles every 4-blocker add. This instance is the true leftover.
"""
...
def main() -> None:
    ...
    for i, u in enumerate(add_verts):
        for w in add_verts[i + 1 :]:
            if adjacent(u, w):
                cnf.append([-add_map[u], -add_map[w]])
    ...
    if sat:
        ...
        lines.append("no 368 in <=3-blocker Hamming-13")
