/* Longer ejection sample around the 367-set (7-surjective plateau).

   More restarts and 4-out / 5-out samples than q1. A 367-set is not a
   new bound. Writes a file only on size >= 368.

   gcc -O3 -o q2/search_ejection q2/search_ejection.c
   ./q2/search_ejection R367.txt
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc, char **argv) {
    ...
    int steps = argc > 2 ? atoi(argv[2]) : 250000;
    int restarts = argc > 3 ? atoi(argv[3]) : 16;
    fill_tables();
    ...
    int four_out_gain = -99, five_out_gain = -99;
    int four_trials = 0, five_trials = 0;
        ...
        for (int t = 0; t < steps; t++) {
            ...
            if (nS > best) {
                ...
                if (nS > global_best) {
                    ...
                    fflush(stdout);
                    if (nS >= 368) {
                        write_set("q2/R_ejection.txt", S, nS);
                        return 0;
                    }
                }
            } else {
            ...
            if (t % 160 == 0 && nS >= 4) {
                ...
                if (kout == 4) {
                    ...
                    if (gain > five_out_gain) five_out_gain = gain;
                    five_trials++;
                }
                if (gain >= 1) {
                    for (int i = 0; i < np; i++) addv(packed[i], sel, blocked, S, &nS);
                    printf("%d-out hit size=%d gain=%d\n", kout, nS, gain);
                    if (nS >= 368) {
                        write_set("q2/R_ejection.txt", S, nS);
                        return 0;
                    }
                } else {
                    for (int i = 0; i < kout; i++) addv(drop[i], sel, blocked, S, &nS);
                }
            }
            if (stuck > 10000) {
                for (int i = 0; i < 10 && nS > 0; i++) {
                    rng = rng * 1664525u + 1013904223u;
                ...
                gen++;
                for (int u = 0; u < NV; u++)
                    if (blocked[u] == 0 && nf < 1024) freed[nf++] = u;
                for (int i = nf - 1; i > 0; i--) {
                    rng = rng * 1664525u + 1013904223u;
                    int j = (int)(rng % (unsigned)(i + 1));
                    int tmp = freed[i];
    ...
    printf("DONE global_best=%d four_out_best_gain=%d four_trials=%d "
           "five_out_best_gain=%d five_trials=%d\n",
           global_best, four_out_gain, four_trials, five_out_gain, five_trials);
    return 0;
}
