#!/usr/bin/env python3
"""Build the explicit Theorem 5.4 partitions of the r=18 and r=20 lifts.
...
the final block.  The result has 2^(m+1)+1 blocks.  This script reconstructs
the complete lift identity before writing labels for the tracked matrices.
"""
...
def build_case(
    ...
    return {
        ...
        "partition_sha256": hashlib.sha256(output_path.read_bytes()).hexdigest(),
        "redundancy": matrix_r,
        "length": matrix_n,
        "partition_blocks": 2 * size + 1,
        "indicator_sets_by_seed_block": {
            str(block): values for block, values in indicator_sets.items()
        },
        "field_modulus_hex": "0x%X" % MODULUS[m],
        "cross_block_covered": cross_covered,
...
def main() -> None:
    ...
    manifest = {
        "format": "covering-lift-partitions-v1",
        "construction": "Theorem 5.4 proof: beta interim blocks, xi split, D last",
        "seed": str(args.seed),
    ...
    for record in records:
        print(
            ...
            "blocks=%(partition_blocks)d cross_block_covered="
