...
/*
 * Independent q15 verifier for ell_2(41,3) <= 26175.
 *
 * This program shares no code with build_qm43.py.  It uses one 8 MiB bitset
 * to exhaust all 2^26 syndromes under the 33-block seed partition, repeats
 * the sweep for its 33-block merge partition, checks that the latter really is a
 * refinement, verifies GF(32), and compares all 26,175 output columns with
 * Construction QM_4^3.  It deliberately does not allocate or sweep 2^41.
 */
...
{
    const uint16_t modulus = UINT16_C(0x25); /* x^5 + x^2 + 1 */
    uint16_t a = left;
    uint16_t product = 0;
    while (right != 0) {
        if (right & 1U)
    ...
    if (product >= FIELD_SIZE)
        fail("GF(32) multiplication escaped the field");
...
static void verify_field(void)
{
    for (int value = 0; value < FIELD_SIZE; ++value) {
        if (gf_multiply((uint8_t)value, 0) != 0 ||
            ...
            fail("GF(32) identity failed at %d", value);
    }
    for (int left = 1; left < FIELD_SIZE; ++left) {
        ...
        if (inverses != 1)
            fail("GF(32) element %d has %d inverses", left, inverses);
    }
}
...
int main(int argc, char **argv)
{
    ...
    struct cover_counts coarse_counts =
        verify_partition(seed, coarse, "33-block seed partition");
    struct cover_counts refined_counts =
        verify_partition(seed, refined, "33-block seed partition");
    ...
    printf(
        ...
        "GF32_modulus=0x25 indicators=33 output_r=%d output_n=%d "
