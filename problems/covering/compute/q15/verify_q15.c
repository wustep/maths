...
/*
 * Independent q15 verifier for ell_2(47,3) <= 104703.
 *
 ...
 * the sweep for its 129-block refinement, checks that the latter really is a
 * refinement, verifies GF(128), and compares all 104,703 output columns with
 * Construction QM_4^3.  It deliberately does not allocate or sweep 2^47.
 */
...
{
    const uint16_t modulus = UINT16_C(0x83); /* x^7 + x + 1 */
    uint16_t a = left;
    uint16_t product = 0;
    while (right != 0) {
        if (right & 1U)
    ...
    if (product >= FIELD_SIZE)
        fail("GF(128) multiplication escaped the field");
...
static void verify_field(void)
{
    for (int value = 0; value < FIELD_SIZE; ++value) {
        if (gf_multiply((uint8_t)value, 0) != 0 ||
            ...
            fail("GF(128) identity failed at %d", value);
    }
    for (int left = 1; left < FIELD_SIZE; ++left) {
        ...
        if (inverses != 1)
            fail("GF(128) element %d has %d inverses", left, inverses);
    }
}
...
int main(int argc, char **argv)
{
    ...
    struct cover_counts refined_counts =
        verify_partition(seed, refined, "129-block seed partition");
    ...
    printf(
        ...
        "GF128_modulus=0x83 indicators=129 output_r=%d output_n=%d "
