...
def read_partition(path: Path, length: int) -> list[int]:
    """Read either a plain label list or the seed partition JSON."""
    if path.suffix == ".json":
        with path.open("r", encoding="utf-8") as handle:
    ...
    assert len(labels) == length, (
        "%s: got %d partition labels, expected %d" %
        (path, len(labels), length))
    blocks = sorted(set(labels))
    assert blocks == list(range(len(blocks))), (
        "%s: partition labels are not contiguous from zero" % path)
    return labels
...
def verify_radius2_partition(
    columns: list[int], labels: list[int], redundancy: int
) -> tuple[int, int]:
    """Exhaustively certify V_{2m} and its explicit 2-partition."""
    space = 1 << redundancy
    ...
    cross_covered = bytearray(space)
    covered[0] = 1
    cross_covered[0] = 1
    for left, column in enumerate(columns):
        covered[column] = 1
        cross_covered[column] = 1
        for right in range(left):
            covered[column ^ columns[right]] = 1
            if labels[left] != labels[right]:
                cross_covered[column ^ columns[right]] = 1
    count = sum(covered)
    assert count == space, "V_%d covers only %d/%d syndromes" % (
        redundancy, count, space)
    partition_count = sum(cross_covered)
    assert partition_count == space, (
        "V_%d partition cross-covers only %d/%d syndromes" %
        (redundancy, partition_count, space))
    # The all-singleton radius-one cover has exactly 1+n syndromes here, so
    # these constituents have covering radius exactly two, as Theorem 7.3
    # requires, rather than merely at most two.
    assert 1 + len(columns) < space
    return count, len(set(labels))
...
def main() -> None:
    ...
    parser.add_argument("--partition-blocks", type=int, required=True)
    ...
    radius2_labels = read_partition(args.radius2_partition, radius2_n)
    radius2_covered, radius2_blocks = verify_radius2_partition(
        radius2_columns, radius2_labels, radius2_r)
    assert radius2_blocks == args.partition_blocks
    ...
    print(
        ...
        "V_partition=%s V_partition_blocks=%d "
        "V_partition_cross_covered=%d/%d "
        "golay_partition_blocks=23 indicator_bound=23<=%d field_modulus=0x%X "
