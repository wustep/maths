...
with opposite algorithms (Python and Rust) that recheck the covering
condition and the partition from the committed matrix files.
\end{abstract}
\endgroup
...
$1024$ syndromes uncovered. The minimum, $9$, is attained by exactly three columns: $381$,
$479$, and $927$. In geometric language, the column set of $\HH$ is a
minimal $1$-saturating set in $PG(9,2)$. The covering-code literature
calls such a code locally optimal.
...
Both verifiers were written by the same person in one session. The
programs share no code and differ in language, algorithm, and parser,
but they are not the work of two people. A third program, kept outside
the repository, ran once after the two verifiers agreed, and it
confirmed every value.
