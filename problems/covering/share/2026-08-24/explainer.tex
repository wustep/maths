...
$(2,0)$-partition into ten blocks, so Construction~$\mathrm{QM}_2^2$
...
and how to re-check every number from the files in the public repository.
...
from the Kaikkonen--Rosendahl $51$-column set of 2003. Claude Opus~5 found
...
$[n,k]_2$ code. When the covering radius is asserted, it is appended
to the parameters: an $[n,k]_2\,R$ code. For $R=2$ the covering
condition on the column set is
\begin{equation}
\label{eq:cover}
...
family. This note gives such a matrix and such a partition.
...
\subsection{Annealing from the 2003 code}
...
a fixed xorshift64 state, the first run of the script reached covering
radius $2$ at proposal $3{,}600{,}281$.

The same method failed at three nearby parameters. At $(r,n)=(8,25)$ the
...
\item The searches at $(10,49)$, $(9,38)$, and $(8,25)$, which left
$7$, $8$, and $3$ syndromes uncovered respectively, are incomplete
searches, not lower bounds.
