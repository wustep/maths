...
\texttt{COMMIT}. Every number is re-checked from the files in that pin.
\end{abstract}
...
The run starts from the best single deletion of the
Kaikkonen--Rosendahl $51$-set: deleting column $419$
($\mathtt{0x1A3}$) leaves $11$ syndromes uncovered. Energy is the
number of uncovered syndromes. Moves that do not raise energy always
pass; moves that raise it by $\Delta$ pass with probability
$e^{-\Delta/T}$ (Metropolis).
...
The first search produced only the matrix. The $10$-block partition,
Construction $\mathrm{QM}_2^2$, and the two verifiers came after, and
the covering radius of $\HH$ is a finite check of those committed
files. The partition search code is not in the repository. The
artifact is \path{result/data/partition_p10.json}.
