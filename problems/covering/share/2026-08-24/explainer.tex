...
% Language/reorg pass; claims unchanged from 16 August 2026.
...
\texttt{736a38f}. Every number is re-checked from the files in that pin.
\end{abstract}
...
$\{1{:}859,\ 2{:}129,\ 4{:}24,\ 5{:}9,\ 6{:}3\}$. Every multiplicity
is positive, which proves \eqref{eq:cover}. Exactly $973$ syndromes
require two columns, so the covering radius is exactly $2$. The
...
Minimality applies to this particular $50$-set. The existence of a
covering $49$-set remains open.
...
length-$51$ code. Minimality of the $10$-block partition remains open.
...
\]
This iteration leaves $r=12,14,16,22,24,26,28,42,44$.
...
The family rows past $m=5$ follow from the bound
$p(\HH_C)\le 2^{m+1}+1$ of~\cite{DMP2025}.
...
published table entry. The search below improves that seed by one
column.

The anneal starts from the best single deletion of this $51$-set:
deleting column $419$ ($\mathtt{0x1A3}$) leaves $11$ syndromes
uncovered. Energy is the number of uncovered syndromes. Moves that
preserve or lower energy always pass; moves that raise it by $\Delta$ pass
...
proposal $3{,}600{,}281$. The search stopped there. Partitioning and
Construction $\mathrm{QM}_2^2$ were separate computations.
...
syndrome that requires two columns. That finite check is the
...
left $8$ of $512$. At $(10,49)$ it left $7$ of $1024$.
...
\path{result/data/partition_p10.json}. That committed file certifies
$p(\HH)=10$. The search found colourings with $16$, $12$, and $11$
blocks on the way down. It found no $9$-block or $8$-block colouring
in that run. Ten blocks is smaller than the computer-searched
...
The two independent verifiers of Section~\ref{sec:verif} are in
\path{result/verify/}.
...
radius of $\HH$ is established by this finite verification.
...
cd maths
git checkout 736a38f
cd problems/covering/share/2026-08-24/result
./run_all.sh
\end{verbatim}
The pipeline needs \texttt{python3} and \texttt{rustc}. After
the checkout it runs offline with those two tools and deterministic
inputs. It executes $74$ named assertions, exits with status $0$, and
prints byte-identical output across runs. A run takes about two
seconds.
Direct links on the pin:
\url{https://github.com/wustep/maths/tree/736a38f/problems/covering/share/2026-08-24/result}
and
\href{https://github.com/wustep/maths/blob/736a38f/problems/covering/share/2026-08-24/result/run_all.sh}{\texttt{result/run\_all.sh}}.
...
Both programs derive their results from the matrix text. After they
agreed, a third program kept outside the repository was run once as a
post-hoc comparison and confirmed every value. The committed replay
is \texttt{./run\_all.sh}.
...
and $(8,25)$ left $7$, $8$, and $3$ syndromes uncovered, respectively.
...
\item \emph{Gaps.} The scope excludes
$r\in\{12,14,16,22,24,26,28,42,44\}$.
\item \emph{Asymptotics.} The result is the upper bound
$f(2)\le 2601/2048$. Whether $f(2)=1$ is open.
