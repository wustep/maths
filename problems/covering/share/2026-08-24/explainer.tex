...
\url{https://github.com/wustep/maths}. Every number can be re-checked
from the files in that public repository.
\end{abstract}
...
Table~\ref{tab:part} are of that form.
...
Minimality applies only to this particular $50$-set; it does not decide
whether a covering $49$-set exists.
...
of~\cite{DMP2025} gives $p(\HH_{KR})=11$ for the length-$51$ code.
Minimality of the $10$-block partition is not claimed.
...
nonzero columns. An array stores, for every syndrome, how many times it
appears as $0$, a singleton, or a pair-sum. Replacing one column
changes only that singleton and the $49$ pair-sums that use it. Most
proposals target an uncovered syndrome $g$: the search offers $g$
...
\subsection{From the matrix}

The first search produced only the matrix. A later session treated that
matrix as a claim to check. It reconstructed the Kaikkonen--Rosendahl
baseline from Theorem~4.3 of~\cite{DMP2025}, found the $10$-block
$(2,0)$-partition, implemented Construction $\mathrm{QM}_2^2$, and
wrote the two verifiers of Section~\ref{sec:verif}. The covering radius
of $\HH$ is then a finite check on those committed files. The
partition search code is not in the repository. The committed artifact
is \path{result/data/partition_p10.json}.
...
Clone the public repository, check out commit \texttt{COMMIT}, and run
...
./run_all.sh
\end{verbatim}
\texttt{COMMIT} is a placeholder for the hash of this note after it is
merged. The pipeline needs \texttt{python3} and \texttt{rustc}. After
...
Direct links at that commit:
\url{https://github.com/wustep/maths/tree/COMMIT/problems/covering/share/2026-08-24/result}
and
\href{https://github.com/wustep/maths/blob/COMMIT/problems/covering/share/2026-08-24/result/run_all.sh}{\texttt{result/run\_all.sh}}.
...
After both programs agreed, a third program, kept outside the
repository, was run once as a post-hoc comparison and confirmed every
value. It is not in the tree and is not part of \texttt{./run\_all.sh}.
...
\path{compute/} & the search, deterministic replay \\
\path{ATTACK.md} & chronological log \\
\path{PROBLEM.md} & the problem statement \\
\path{RESEARCH.md} & literature log \\
\path{WALKTHROUGH.md} & discovery notes \\
\bottomrule
\end{tabular}
\end{center}
